...
const ArticleCard = ({ article }) => {
  ...
  return (
    <Link href={route('farmer.articles.show', article.slug)} className="block h-full">
      <div className="bg-white rounded-lg shadow-md overflow-hidden h-full flex flex-col transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
        ...
        <div className="p-4 flex-1 flex flex-col">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs text-neutral-dark">{formattedDate}</span>
          </div>
          
          <h3 className="text-lg font-semibold text-neutral-darkest mb-2 line-clamp-2">{article.title}</h3>
          
          <p className="text-sm text-neutral-dark mb-4 line-clamp-3">{article.excerpt}</p>
