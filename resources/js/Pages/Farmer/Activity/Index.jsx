...
const ActivityHistoryPage = ({ auth }) => {
    ...
    const loadActivities = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `/api/farmer/activities?page=${currentPage}&type=${activeTab}`,
            );
    ...
    const getStatusBadge = (status) => {
        ...
        return (
            <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${statusClasses[status.toLowerCase()] || "bg-neutral-light text-neutral-dark"}`}
            >
