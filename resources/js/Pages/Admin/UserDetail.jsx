...
import { UserCircle, Calendar, Mail, Tag, ArrowLeft, Check, X, Phone, MapPin, Award, Store, Tractor, UserCheck, UserX } from 'lucide-react';
...
export default function UserDetail() {
  ...
  const handleApprove = async () => {
    try {
      ...
      const response = await axios.post(`/admin/users/${user.id}/approve`);
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const response = await axios.post(`/admin/users/${user.id}/toggle-status`, {
        isActive: !isActive,
      });
      ...
      <div className="mt-8">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          {doctor.license_number && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Nomor Lisensi</p>
          ...
          {doctor.practice_address && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Alamat Praktik</p>
              <p className="font-medium">{doctor.practice_address}</p>
            </div>
          )}
          
          {doctor.years_experience && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Pengalaman (Tahun)</p>
              <p className="font-medium">{doctor.years_experience}</p>
            </div>
          )}

          {doctor.rejection_reason && status === 'rejected' && (
            <div className="mb-3 p-3 bg-red-50 border border-red-100 rounded-md">
              <p className="text-sm text-red-500 font-medium">Alasan Penolakan</p>
              <p className="text-red-700">{doctor.rejection_reason}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          {shop.shop_description && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Deskripsi Toko</p>
              <p className="font-medium">{shop.shop_description}</p>
            </div>
          )}
          
          {shop.owner_id_number && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Nomor Identitas Pemilik</p>
              <p className="font-medium">{shop.owner_id_number}</p>
            </div>
          )}
    ...
    return (
      <div className="mt-8">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          {farmer.farm_name && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Nama Peternakan</p>
          ...
          {farmer.farm_location && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Lokasi Peternakan</p>
          ...
          {farmer.farm_size && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Ukuran Peternakan</p>
          ...
          {farmer.years_experience && (
            <div className="mb-3">
              <p className="text-sm text-gray-500">Pengalaman (Tahun)</p>
              <p className="font-medium">{farmer.years_experience}</p>
            </div>
          )}
        </div>
      </div>
  ...
  const renderStatusBadge = () => {
    ...
    return (
      <span className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeColor(status)}`}>
  ...
  const renderActionButtons = () => {
    if ((user.role !== 'doctor' && user.role !== 'shop') || status !== 'pending') {
      return null;
    }
  ...
  const renderActivationButton = () => {
    ...
    if ((user.role === 'doctor' || user.role === 'shop') && status !== 'verified' && user.role !== 'farmer') {
      return null;
    }
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
            <div className="flex items-center">
              ...
              {status === 'pending' ? renderActionButtons() : renderActivationButton()}
            </div>
          </div>
